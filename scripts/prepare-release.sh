...
if [ ! -f ~/.tauri/voiceit.key ]; then
  echo "❌ Private key not found at ~/.tauri/voiceit.key"
  echo "Run: cd tauri && bun tauri signer generate -w ~/.tauri/voiceit.key"
  exit 1
fi

if [ ! -f ~/.tauri/voiceit.key.pub ]; then
  echo "❌ Public key not found at ~/.tauri/voiceit.key.pub"
  exit 1
fi
...
export TAURI_SIGNING_PRIVATE_KEY="$(cat ~/.tauri/voiceit.key)"
