...
def find_binary() -> Optional[Path]:
    ...
    candidates = [
        DIST_DIR / "voiceit-server-cuda" / f"voiceit-server-cuda{exe}",
        DIST_DIR / f"voiceit-server{exe}",
    ]
    for c in candidates:
...
def write_reports(
    ...
    lines = [
        f"# VoiceIt E2E — {plat} — {started_at.strftime('%Y-%m-%d %H:%M UTC')}",
        "",
        f"Binary: `{binary}`  ",
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="VoiceIt E2E model generation test")
    p.add_argument("--binary", type=Path, help="Path to voiceit-server binary (overrides auto-detect)")
...
def main() -> int:
    ...
    # Tempdir + log path
    data_dir = Path(tempfile.mkdtemp(prefix="voiceit-e2e-"))
