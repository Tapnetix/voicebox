"""Stdio → Streamable HTTP bridge for the VoiceIt MCP server.
...
the client spawns us as ``voiceit-mcp``; we proxy every JSON-RPC frame
to http://127.0.0.1:17493/mcp/ and stream responses back out.

All the real work (tools, models, inference) lives in the VoiceIt server
