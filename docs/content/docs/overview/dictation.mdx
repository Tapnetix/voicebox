...
a chord, talk, release — VoiceIt transcribes what you said with Whisper,
...
<Callout type="info">
  ...
  per-profile personality modes. It's the "input" half of VoiceIt's voice I/O
...
<Steps>
  ...
  <Step title="Text lands in your app">
    If auto-paste is enabled and VoiceIt has Accessibility permission, the
...
VoiceIt ships two chord behaviors out of the box:
...
- **Left vs right modifier badges.** When you hold keys into the chord
  picker, VoiceIt records whether each modifier is the left or right variant.
...
Once transcription finishes, VoiceIt can synthesize a native paste into
...
- Whisper loop hallucinations — VoiceIt strips repeated tokens (six or
...
- **UAC / UIPI caveat.** Synthetic paste into an *elevated* window from a
  non-elevated VoiceIt is blocked by Windows itself. Run VoiceIt elevated
...
A few cases where VoiceIt deliberately does *not* synthesize a paste:

- **Focus was inside VoiceIt** when the chord started. The transcript goes
  to the Captures tab so a dictation-into-VoiceIt round-trip doesn't
