...
description: "Run VoiceIt as a headless server with a web UI using Docker"
---

## Overview

VoiceIt can run as a Docker container with a full web UI -- no desktop app required. This is ideal for headless servers, shared GPU machines, or self-hosted deployments.
...
Open [http://localhost:17493](http://localhost:17493) in your browser. The full VoiceIt UI is served directly from the backend.
...
services:
  voiceit:
    ...
    volumes:
      ...
      - huggingface-cache:/home/voiceit/.cache/huggingface
    environment:
      - LOG_LEVEL=info
    deploy:
      resources:
...
| `VOICEIT_MODELS_DIR` | (HuggingFace cache) | Custom path for model storage |
| `VOICEIT_CORS_ORIGINS` | (local origins) | Additional CORS origins, comma-separated |
...
| `voiceit-data` | `/app/data` | Profiles, database, cache |
| `huggingface-cache` | `/home/voiceit/.cache/huggingface` | Downloaded models (persists across rebuilds) |
...
- **Non-root user** -- the server runs as `voiceit`, not `root`
...
For production deployments, put VoiceIt behind nginx or Caddy with TLS and authentication:

```nginx
server {
    listen 443 ssl;
    server_name voiceit.example.com;

    ssl_certificate /etc/ssl/certs/voiceit.pem;
    ssl_certificate_key /etc/ssl/private/voiceit.key;
...
If you see `{"message": "voiceit API", ...}` instead of the web UI, the frontend build may have failed during the Docker build. Check the build logs:
