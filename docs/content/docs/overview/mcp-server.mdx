...
VoiceIt ships a built-in **Model Context Protocol** server so local AI
agents can call your VoiceIt install directly: speak text in a voice
profile, transcribe audio, and list captures or profiles. The server runs
inside the same process as the rest of VoiceIt and is mounted at `/mcp`
over Streamable HTTP.

Agent asks to speak → VoiceIt plays audio on your speakers → an on-screen
...
VoiceIt → Settings → MCP. The value is just an identifier for the
per-client voice binding — not a secret, not a credential.

### Clients that only speak stdio

A stdio shim binary `voiceit-mcp` is bundled with the desktop app. Point
...
<Tabs items={["macOS", "Windows", "Linux"]}>
  ...
  ```json
  {
    "mcpServers": {
      "voiceit": {
        "command": "C:\\Program Files\\VoiceIt\\voiceit-mcp.exe",
        "env": { "VOICEIT_CLIENT_ID": "claude-desktop" }
      }
    }
  }
  ...
  ```json
  {
    "mcpServers": {
      "voiceit": {
        "command": "/opt/voiceit/voiceit-mcp",
        "env": { "VOICEIT_CLIENT_ID": "claude-desktop" }
      }
    }
  }
...
The shim waits up to 30 seconds for the VoiceIt backend to come up, then
proxies JSON-RPC from stdio over Streamable HTTP. VoiceIt must be running
...
| Tool | Use |
|---|---|
| `voiceit.speak` | Speak text in a voice profile. Returns a `generation_id` to poll. |
| `voiceit.transcribe` | Whisper transcription of base64 audio or an absolute local path. |
| `voiceit.list_captures` | Recent captures with transcripts, paginated. |
...
Every call to `voiceit.speak` (and `POST /speak`) resolves the voice profile
in this order:
...
<Steps>
  ...
  <Step title="Per-client binding">
    Looked up by the `X-VoiceIt-Client-Id` header. Managed in
    **VoiceIt → Settings → MCP**. Lets you pin Claude Code to Morgan,
...
VoiceIt → Settings → MCP shows one row per `client_id` VoiceIt has heard
...
| `default_personality` | When true, `voiceit.speak` routes through the profile's personality LLM (rewrite) by default. |
...
silent background TTS is a trust hazard, so VoiceIt always shows what's
...
Start with `voiceit.list_profiles` to confirm wiring, then
`voiceit.speak` for end-to-end — you should hear audio and see the
generation land in the Captures tab.

<Callout type="info">
  If an agent can't reach the server, the first thing to check is that
  VoiceIt is running — the backend only listens while the desktop app is
...
- **Localhost only.** The server binds to `127.0.0.1`. If you ever point
  VoiceIt at a non-loopback interface (e.g. remote-mode over a trusted
...
- **No auth today.** Any process that can connect to your loopback can
  call MCP. That's the same trust boundary as the rest of VoiceIt's REST
...
- **Voice cloning consent applies.** See [Voice Cloning](/overview/voice-cloning#limitations)
  — an agent being able to call `voiceit.speak` in someone's voice
...
- **Lifespan:** mounting FastMCP requires the `lifespan=` kwarg on
  ...
  with FastMCP's Streamable HTTP session manager. The VoiceIt app.py
