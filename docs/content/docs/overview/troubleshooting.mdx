...
description: "Common issues and solutions for VoiceIt"
---

This guide covers common issues you might encounter when using or developing VoiceIt, along with solutions.
...
xattr -cr /Applications/VoiceIt.app
```
...
<AccordionGroup>
  ...
  <Accordion title="Permission Issues">
    ...
    ```bash
    # macOS/Linux
    chmod +x ~/Library/Application\ Support/com.tapnetix.voiceit/backend/voiceit-server
    ```
  </Accordion>
  ...
  <Accordion title="Check Logs">
    ...
    **macOS:**
    ```bash
    tail -f ~/Library/Application\ Support/com.tapnetix.voiceit/logs/server.log
    ```

    **Windows:**
    ```bash
    type %APPDATA%\com.tapnetix.voiceit\logs\server.log
    ```
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Install flash-attn on Windows (community wheels)">
    ...
    Alternatively, run VoiceIt's backend inside WSL2 and use the standard Linux wheels.
  </Accordion>
</AccordionGroup>
...
**Solutions:**
- Close all VoiceIt instances
- Delete the lock file:
  ```bash
  # macOS
  rm ~/Library/Application\ Support/com.tapnetix.voiceit/data/voiceit.db-shm
  rm ~/Library/Application\ Support/com.tapnetix.voiceit/data/voiceit.db-wal
  ```
...
```bash
# macOS
rm ~/Library/Application\ Support/com.tapnetix.voiceit/data/voiceit.db

# Windows
del %APPDATA%\com.tapnetix.voiceit\data\voiceit.db
```
...
<AccordionGroup>
  ...
  <Accordion title="Check GPU Selection">
    If you have multiple GPUs, ensure VoiceIt is using the right one.
...
3. **Join Discord:** [discord.gg/voiceit](https://discord.gg/voiceit) (coming soon)
