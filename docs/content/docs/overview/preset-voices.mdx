...
Some VoiceIt engines ship with a curated set of pre-built voices. Instead of cloning from your own audio sample, you pick a voice from a fixed catalog and the model speaks in that voice. No recording, no upload, no per-voice training required.
...
Kokoro is the smallest engine in VoiceIt at 82M parameters. It runs at CPU realtime with negligible VRAM, making it the best option for lightweight local inference. Voices are pre-built style vectors trained into the model — there's no concept of cloning here.
...
- Preset voices can't be exported to use in other VoiceIt installations as audio (only as profile metadata pointing to the same engine + voice ID)
