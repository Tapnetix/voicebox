...
description: "How VoiceIt uses your GPU — auto-detection, manual setup, troubleshooting"
---

## Overview

VoiceIt auto-detects available accelerators on first launch and picks the fastest backend it can use. For most people this just works — open the app and you're already on the right backend.

This page is for the cases where it doesn't:

- You have a GPU but VoiceIt is running on CPU
...
On M-series Macs, VoiceIt ships an MLX-optimized backend that uses the Apple Neural Engine. It's **4-5x faster** than the PyTorch (CPU/Metal) path for supported engines.
...
VoiceIt doesn't bundle CUDA into the main installer (it would balloon downloads to multi-gigabyte territory for users who don't have an NVIDIA GPU). Instead, when you first need it, the app downloads a separate **CUDA backend binary** that contains the PyTorch + CUDA runtime.
...
<Steps>
  ...
  <Step title="Click Install">
    ...
    - **Server core** (~200-400 MB) — versioned with each VoiceIt release
    - **CUDA libs** (~4 GB) — the heavy PyTorch + CUDA DLLs, versioned independently
  </Step>
  <Step title="Restart">
    VoiceIt restarts to swap in the CUDA backend
  </Step>
</Steps>

<Callout type="info">
  The split-archive design (added in v0.4) means most VoiceIt upgrades only redownload the small server-core archive. The 4 GB libs archive is only refreshed when the underlying CUDA toolkit or torch major version changes.
</Callout>

### Auto-update

When a new VoiceIt release ships, the GPU panel checks if the bundled server-core matches the installed CUDA version. If only the core changed (typical), it pulls the new core in the background. If the libs version changed (rare — only happens on cu126 → cu128 type bumps), you'll be prompted to confirm the larger download.

## RTX 50-series / Blackwell

VoiceIt 0.4 added explicit RTX 50-series support:
...
1. Make sure you're on VoiceIt **≥ 0.4.0** (Settings → About)
...
VoiceIt auto-detects Arc GPUs and routes through Intel's PyTorch XPU backend (powered by IPEX — Intel Extension for PyTorch). No extra installation step beyond the standard VoiceIt install.
...
ROCm provides PyTorch GPU acceleration on AMD discrete GPUs. VoiceIt auto-configures `HSA_OVERRIDE_GFX_VERSION` for common cards that need the override.
...
If detection fails, set the variable manually before launching VoiceIt:
...
VoiceIt 0.4 added a runtime check that compares your GPU's compute capability against the architectures the bundled PyTorch was compiled for. If they don't match, you'll see:
...
1. **Update VoiceIt** — newer releases ship newer PyTorch with broader arch support
...
3. **For bleeding-edge GPUs (newer than current Blackwell):** install PyTorch nightly manually:
   ...
   ```
   Then point VoiceIt at that environment via [Remote Mode](/overview/remote-mode) until stable PyTorch catches up.
4. **Fall back to CPU** temporarily — set `VOICEIT_FORCE_CPU=1` before launching

## CPU-Only Fallback

When no GPU is available (or you've forced it off), VoiceIt runs the PyTorch CPU backend. Expect:
...
<AccordionGroup>
  ...
  <Accordion title="'no kernel image is available' / 'CUDA error'">
    ...
    1. Update to VoiceIt ≥ 0.4.0 (Blackwell support added there)
  ...
  <Accordion title="MPS fallback errors on macOS">
    Some operations don't have a Metal implementation. VoiceIt sets `PYTORCH_ENABLE_MPS_FALLBACK=1` for engines that need it (notably Kokoro), but if you launch from a custom env, set it manually:
    ```bash
