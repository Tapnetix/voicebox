...
The model runs through the same backend split VoiceIt already uses for TTS
...
<Steps>
  ...
  <Step title="Speak in character — toggle persona rewrite">
    ...
    generate — VoiceIt runs the text through the personality LLM first,
...
- **Agents that speak in a voice you own.** Combine the persona toggle with
  ...
  personality. The agent calls `voiceit.speak({ text, profile, personality:
  true })` and VoiceIt rewrites the text in character before speaking.
...
`voiceit.speak` tool uses when you pass `personality: true`. Scripts and
