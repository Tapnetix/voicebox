---
title: "TTS Engines"
description: "How to add new text-to-speech engines to VoiceIt"
---
...
- **`voiceit-server`** (CPU) — excludes all `nvidia.*` packages to avoid bundling ~3 GB of CUDA DLLs
- **`voiceit-server-cuda`** — includes `torch.cuda` and `torch.backends.cudnn`
