---
title: "Building"
description: "How VoiceIt is built for production"
---

## Overview

VoiceIt uses a two-stage build process:
...
cp dist/voiceit-server ../tauri/src-tauri/binaries/voiceit-server-${PLATFORM}
```
...
```json
{
  "tauri": {
    "bundle": {
      "externalBin": ["binaries/voiceit-server"]
    }
  }
}
```

Tauri looks for `voiceit-server-${PLATFORM}` in `src-tauri/binaries/` and bundles it.
...
3. Package with `scripts/package_cuda.py` into two archives:
   - `voiceit-server-cuda.tar.gz` — server core (~945 MB)
...
<AccordionGroup>
  ...
  <Accordion title="Tauri can't find sidecar">
    Ensure binary exists at `tauri/src-tauri/binaries/voiceit-server-${PLATFORM}` before running Tauri build.
  </Accordion>
</AccordionGroup>
