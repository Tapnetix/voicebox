---
title: "Auto-Updater"
description: "How VoiceIt automatic updates work"
---

## Overview

VoiceIt uses Tauri's built-in auto-updater to deliver signed updates to users. The system verifies updates cryptographically before installation.

## How It Works

When VoiceIt launches (in production Tauri builds only), it checks GitHub Releases for a `latest.json` manifest. If a newer version is available:
...
```json
{
  ...
  "platforms": {
    "darwin-aarch64": {
      ...
      "url": "https://github.com/jamiepine/voicebox/releases/download/v0.2.0/voiceit_0.2.0_aarch64.app.tar.gz"
    },
    "darwin-x86_64": {
      "signature": "base64_encoded_signature",
      "url": "https://github.com/jamiepine/voicebox/releases/download/v0.2.0/voiceit_0.2.0_x64.app.tar.gz"
    },
    "linux-x86_64": {
      "signature": "base64_encoded_signature",
      "url": "https://github.com/jamiepine/voicebox/releases/download/v0.2.0/voiceit_0.2.0_amd64.AppImage"
    },
    "windows-x86_64": {
      "signature": "base64_encoded_signature",
      "url": "https://github.com/jamiepine/voicebox/releases/download/v0.2.0/voiceit_0.2.0_x64_en-US.msi"
    }
  }
}
```
...
1. **Generate keys** (one-time setup):
   ```bash
   bun tauri signer generate -w ~/.tauri/voiceit.key
   ```
   This creates:
   - Private key: `~/.tauri/voiceit.key` (stored in GitHub Secrets, never committed)
   - Public key: `~/.tauri/voiceit.key.pub` (pasted into `tauri.conf.json`)
...
- `TAURI_SIGNING_PRIVATE_KEY` - Content of `~/.tauri/voiceit.key`
...
- CUDA server core: ~945 MB (versioned with each VoiceIt release)
...
- **Server core** (`voiceit-server-cuda.tar.gz`) — the Python server + PyTorch code, changes every release.
...
This means most VoiceIt upgrades only re-download the ~945 MB server core, not the full ~2.5 GB bundle.
...
5. **Restart** — The VoiceIt server restarts and swaps in the CUDA backend.
...
{data_dir}/backends/cuda/
  voiceit-server-cuda.exe    # Windows
  voiceit-server-cuda        # macOS/Linux
...
event: progress
data: {"current": 52428800, "total": 945000000, "filename": "voiceit-server-cuda.tar.gz", "status": "downloading"}
```
...
- `voiceit-server-cuda.tar.gz` — server-core archive
- `voiceit-server-cuda.tar.gz.sha256` — checksum
