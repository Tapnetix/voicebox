...
build-server: _ensure-venv
    ...
    Copy-Item "backend/dist/voiceit-server.exe" "{{ tauri_dir }}/src-tauri/binaries/voiceit-server-$triple.exe" -Force; \
    Write-Host "Copied sidecar: voiceit-server-$triple.exe"
...
build-server-cuda: _ensure-venv
    ...
    $dest = "$env:APPDATA/com.tapnetix.voiceit/backends/cuda"; \
    if (Test-Path $dest) { Remove-Item -Recurse -Force $dest }; \
    New-Item -ItemType Directory -Path $dest -Force | Out-Null; \
    Copy-Item "backend/dist/voiceit-server-cuda/*" $dest -Recurse -Force; \
...
[unix]
db-reset:
    rm -f {{ backend_dir }}/data/voiceit.db
    just db-init

[windows]
db-reset:
    if (Test-Path "{{ backend_dir }}/data/voiceit.db") { Remove-Item -Force "{{ backend_dir }}/data/voiceit.db" }
    just db-init
